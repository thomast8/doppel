...
| A planted app bundle in a scan root running its own config | Addressed. `list`, `update check` and `adopt` look for managed apps in `~/Applications` and `/Applications`, so the config they read comes from a bundle anything running as you can write, including one downloaded and never opened. Those configs are parsed, never sourced, so nothing in one runs during a scan. Adoption additionally refuses any config that is not the plain list of `%q`-quoted settings `create` writes, because the engine inside a bundle still sources its own copy: a line the parser skips, or a value that is more than one token, would mean one thing to what validates it and another to what launches it. |
